...
export default function EditFamilyPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'فشل تحديث الحساب');
      }
  ...
  if (fetching) return <div style={{ padding: '2rem', textAlign: 'center' }}>جاري تحميل بيانات الحساب...</div>;
      ...
      <main className="main-content">
        <form onSubmit={handleSave}>
          <header style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h1 style={{ fontSize: '1.8rem', color: 'var(--primary)', fontWeight: 'bold' }}>تعديل حساب الأهل</h1>
              <p style={{ color: 'var(--text-muted)' }}>تعديل بيانات الدخول الخاصة بالأسرة</p>
            </div>
          ...
          <div className="card">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem', fontWeight: '600' }}>كلمة المرور (اتركها فارغة لعدم التغيير)</label>
