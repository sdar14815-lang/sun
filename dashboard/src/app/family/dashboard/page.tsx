...
import FamilyHealingCorner from '@/components/FamilyHealingCorner';
...
function CircularProgress({ percentage, color }: { percentage: number; color: string }) {
  ...
  const gradId = `fp-progress-grad-${percentage}-${color.replace('#', '')}`;
      ...
      <svg viewBox="0 0 56 56">
        <defs>
          <linearGradient id={gradId} x1="100%" y1="0%" x2="0%" y2="100%">
            ...
            <stop offset="100%" stopColor="var(--fp-accent)" />
          </linearGradient>
        </defs>
...
// ── Lightweight Particle Confetti Component (With Auto-stop to save battery) ──
function CelebrationConfetti() {
  const [show, setShow] = useState(true);
  
  useEffect(() => {
    const timer = setTimeout(() => setShow(false), 8000); // Stop after 8 seconds
...
export default function FamilyDashboardPage() {
  ...
  const handleSubscribe = async () => {
    if (typeof window !== 'undefined') {
      ...
      try {
        // 3. Request permission directly using Native API to preserve user gesture
    ...
    try {
      ...
      if (profError) {
        ...
        if (profError.code === '42501' || profError.message?.toLowerCase().includes('permission') || profError.message?.toLowerCase().includes('policy')) {
          setStatusError('ليس لديك صلاحية لعرض هذه الصفحة (مشكلة في سياسات الوصول RLS)');
        } else if (profError.code === 'PGRST116') {
          setStatusError('لم يتم العثور على ملف تعريفي (Profile) مرتبط بحسابك في قاعدة البيانات. يرجى التواصل مع الإدارة لربط الحساب.');
        } else {
          setStatusError(`حدث خطأ أثناء تحميل بيانات الحساب: ${profError.message} (كود: ${profError.code})`);
        }
        setLoading(false);
        return;
      }
  ...
  return (
    <div style={{ minHeight: '100vh', paddingBottom: '6rem', background: 'var(--fp-surface)' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: 'clamp(0.875rem, 4vw, 2rem)' }}>
        ...
        {/* ── Premium Healing Corner ── */}
        <FamilyHealingCorner resident={residents[0]} />
